...
def res_manipulator(enc_a,
    ...
    for i in range(num_conv):
        ...
        if use_shape_deconv:
            diff = tf.nn.relu(conv2d(diff, 3, k, 1, padding='VALID', name=cname + 'c'))
        else:
    ...
    for i in range(num_aft_conv):
        ...
        if use_shape_deconv:
            diff = conv2d(diff, 3, 3, 1, padding='VALID', name=cname + 'c')
        else:
            diff = conv2d(diff, layer_dims, 3, 1, padding='VALID', name=cname + 'c')

    for i in range(num_resblk):
        if use_shape_deconv:
            diff = residual_block(diff, 3, 3, 1, name='mani_resblk{}'.format(i))
        else:
