...
from ops import conv2d_transpose_strided,weight_variable,bias_variable,deconv2d
...
class MagNet3Frames(object):
    ...
    def __init__(self, sess, name, arch_config):
        ...
        self.use_shape_deconv = arch_config["ynet_3frames"]["use_shape_deconv"]
        self.decoder_dims = self.texture_dims + self.shape_dims
        self.probe_pt = {}
        self.save_out_shape_enc=[]
        self.manipulator = partial(res_manipulator,
                                   ...
                                   use_shape_deconv = self.use_shape_deconv)
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            with tf.variable_scope('manipulator'):
                self.out_shape_enc = self.manipulator(self.shape_a,
                                                      self.shape_b,
                                                      amplification_factor)
                self.tmp_out = np.array(self.out_shape_enc)

            with tf.variable_scope('decoder'):
                return self._decoder(self.texture_b, self.out_shape_enc), self.out_shape_enc
    ...
    def inference(self, frameA, frameB, amplification_factor):
        ...
        out_amp, diff = self.sess.run(self.test_output,
